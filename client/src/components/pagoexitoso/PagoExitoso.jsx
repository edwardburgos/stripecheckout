    ...
    useEffect(() => {
        async function informaciondepago() {
            ...
            let eventos = detalleCompra.map(e => {
                ...
                let nombreDia = fechaActual.getDay() - 1;
            ...
            <div className={s.detalleContainer}>
                {
                    productosComprados.length ?
                        productosComprados.map((e, i) => <Item key={i} id={e.id} imagen={e.imagen} nombre={e.nombre} precioDescuento={e.precioDescuento} moneda={e.moneda} dia={e.dia} horaInicio={e.horaInicio} horaFin={e.horaFin} profesor={e.profesor} comprado={true}></Item>)
                        :
